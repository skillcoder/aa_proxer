...
function piper(client, cb) {
   ...
   * Returns nothing
   */
  function pipe(client, host, forwardResponse, buffer) {
    if (typeof buffer == 'undefined') {
		buffer = new BufferedStream;
		buffer.pause();
	}
  ...
  function onConnect(client) {
    ...
    /**
     * Public: Forward the client to a host, sending the response back to the client
     *
     * host   - a host that parseHost can understand
     */
	  function forward(host, cb) {
		  var server = null;
		  ...
		  if (wasConnectionForwarded) {
			  ...
			  server = pipe(client, host, false, buffer)
		  } else {
			  server = pipe(client, host, wasConnectionForwarded = true, buffer)
		  }
    ...
    function duplicate(host) {
      return pipe(client, host)
    }

    // if the user defined a callback, call that
    cb && cb(forward, duplicate)
  }
